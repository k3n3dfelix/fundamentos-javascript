...
let arrayStrings =  [];
let stringsAmount = 0;
let booleansItered = 0;
let arrayIteredAmount = 0
...
for(let i = 0; i < randomValues.length; i++){
    ...
    if(isItemAstring){
      stringsAmount++;
      arrayStrings.push(randomValues[i]);
    }
    arrayIteredAmount++;
}

let lastItem = arrayStrings[arrayStrings.length -1];
const fourStrings = arrayStrings.join(', ').replace(`, ${lastItem}`, ` e ${lastItem}`)

console.log(`As primeiras 4 strings são: ${fourStrings}`)
console.log(`Até que as primeiras 4 strings fossem iteradas, ${booleansItered} booleans foram iterados`);
console.log(`O array foi iterado por ${arrayIteredAmount} vezes.`)
